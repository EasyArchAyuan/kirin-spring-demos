...
public class AsyncTaskExecutor {
    ...
    private static ThreadPoolExecutor createThreadPoolExecutor(Environment environment) {
        ...
        String coreSizeStr = environment.getProperty("com.example.demo.asyncInitBeanCoreSize");
        if (coreSizeStr != null) {
            threadPoolCoreSize = Integer.parseInt(coreSizeStr);
        }

        int threadPoolMaxSize = CPU_COUNT + 1;
        String maxSizeStr = environment.getProperty("com.example.demo.asyncInitBeanMaxSize");
        if (maxSizeStr != null) {
