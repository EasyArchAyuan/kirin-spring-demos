...
public class AsyncInitBeanFactoryPostProcessor implements BeanFactoryPostProcessor, ApplicationContextAware, EnvironmentAware {
    ...
    private String moduleName = "kirin";

    @Override
    public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
        Arrays.stream(beanFactory.getBeanDefinitionNames()).collect(Collectors.toMap(Function.identity(), beanFactory::getBeanDefinition)).forEach((key, value) -> scanAsyncInitBeanDefinition(key, value, beanFactory));
    }
...
//            SofaLogger.error(ErrorCode.convert("01-02001", beanId), throwable);
        ...
        if (candidateMethods.size() == 1) {
            SofaAsyncInit sofaAsyncInitAnnotation = candidateMethods.get(0).getAnnotation(SofaAsyncInit.class);
...
//                        declaringClass.getCanonicalName()));
                    throw new FatalBeanException("报错了");
                }
            }
        }
    }
    ...
    private void scanAsyncInitBeanDefinitionOnClass(String beanId, Class<?> beanClass, BeanDefinition beanDefinition, ConfigurableListableBeanFactory beanFactory) {
        ...
        SofaAsyncInit sofaAsyncInitAnnotation = AnnotationUtils.findAnnotation(beanClass, SofaAsyncInit.class);
        registerAsyncInitBean(beanId, sofaAsyncInitAnnotation, beanDefinition);
    }
    ...
    private void registerAsyncInitBean(String beanId, SofaAsyncInit sofaAsyncInitAnnotation, BeanDefinition beanDefinition) {
        ...
        PlaceHolderAnnotationInvocationHandler.AnnotationWrapperBuilder<SofaAsyncInit> wrapperBuilder = PlaceHolderAnnotationInvocationHandler.AnnotationWrapperBuilder.wrap(sofaAsyncInitAnnotation).withBinder(binder);
